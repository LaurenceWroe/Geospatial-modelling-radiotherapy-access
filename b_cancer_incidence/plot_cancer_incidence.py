#!/usr/bin/env python3
"""
Generate a cancer incidence (not treated by radiotherapy just diagnosed) country map for a selected cancer type by multiplying the
...
def multiply_population_by_proportion(
    population_raster_path: str,
    proportion: float,
) -> Tuple[np.ndarray, np.ndarray]:
    """
    Load population raster and return population * proportion array.
    This reflects cancer incidence by type, not filtered by radiotherapy fraction.
    """
    ...
    # Multiply by proportion only (not fraction)
    result = population.astype(np.float64) * float(proportion)
...
def generate_cancer_type_map(
    ...
    proportion, _ = fractions[cancer_key]
    ...
    # Calculation
    population, array = multiply_population_by_proportion(population_raster_path, proportion)
   ...
   # ax.set_title(title)
    ...
    cbar = plt.colorbar(im, ax=ax)
    cbar.set_label("Estimated cancer incidence density (cases/km²)")
    ax.set_title(title)
    ax.set_xlabel("Longitude")
    ax.set_ylabel("Latitude")
    plt.tight_layout()
